...
import { Loader2, User, Lock, Eye, EyeOff, Mail } from '@lucide/vue';
import PasskeyRegistrationDialog from '@/components/PasskeyRegistrationDialog.vue';
import PasswordStrengthIndicator from '@/components/PasswordStrengthIndicator.vue';
import { apiRoutes } from '#shared/apiRoutes';
import type { ApiRegistrationPayload, ApiResponse } from '~~/types/api';
...
function handleFieldBlur(field: 'username' | 'password' | 'confirmPassword') {
  let value = '';
  if (field === 'username') value = username.value;
  else if (field === 'password') value = password.value;
  else value = confirmPassword.value;
...
function handleFieldInput(field: 'username' | 'password' | 'confirmPassword') {
...
async function registerWithPassword() {
  ...
  try {
    ...
    const response = await apiRequest<ApiResponse<ApiRegistrationPayload>>(
      apiRoutes.auth.registerPassword,
      {
        method: 'POST',
        body: {
          username: username.value,
    ...
    error.value = parseApiError(err, 'Registration failed').message;
  } finally {
  ...
  >
    <Card class="w-full max-w-md mx-4">
      <CardHeader class="text-center relative">
        ...
        <CardDescription> Join Gromet Reader and start your reading journey </CardDescription>
      </CardHeader>
      ...
      <CardContent>
        ...
        <form class="space-y-4" @submit.prevent="registerWithPassword">
          ...
          <Button type="submit" class="w-full h-11" :disabled="isLoading || !isFormValid">
        ...
        <div class="text-center mt-6 space-y-3">
          ...
          <div class="text-sm text-gray-600 dark:text-gray-300">
            ...
            <nuxt-link to="/auth/login" class="text-primary hover:underline font-medium">
              Sign In
            </nuxt-link>
          </div>
        </div>
