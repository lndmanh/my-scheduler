...
function readFailureEnvelope(value: unknown): ApiResponseFailure | null {
  ...
  return {
    success: false,
    error:
      Object.keys(fieldErrors).length > 0 || Object.keys(details).length > 0
        ? {
            code,
            message,
...
export function parseApiError(
  ...
  const envelope =
    readFailureEnvelope(nestedData) ?? readFailureEnvelope(data) ?? readFailureEnvelope(error);
  ...
  const bareError =
    readBareErrorObject(error) ?? readBareErrorObject(data) ?? readBareErrorObject(nestedData);
  ...
  const status = errorRecord
    ...
    : dataRecord
      ? readErrorStatus(dataRecord)
      : undefined;
  const statusText = errorRecord
    ? (readErrorStatusText(errorRecord) ??
      (dataRecord ? readErrorStatusText(dataRecord) : undefined))
    : dataRecord
      ? readErrorStatusText(dataRecord)
      : undefined;
  ...
  return {
    ...
    code: nestedRecord
      ? (readString(nestedRecord, 'code') ??
        (dataRecord ? readString(dataRecord, 'code') : undefined) ??
        'ERROR')
      : dataRecord
        ? (readString(dataRecord, 'code') ?? 'ERROR')
        : 'ERROR',
    status,
    statusText,
    fieldErrors: Object.keys(nestedFieldErrors).length > 0 ? nestedFieldErrors : bodyFieldErrors,
    details: Object.keys(nestedDetails).length > 0 ? nestedDetails : bodyDetails,
  };
}
